#!/usr/bin/env python3
"""
ROS2 inference/record wrapper for lerobot.

Drop-in replacement for lerobot-record that publishes:
...
This script monkey-patches the lerobot record loop to intercept robot
...
All lerobot-record arguments are supported and passed through unchanged.
"""
...
# Saved reference to the ORIGINAL record_loop (before monkey-patching)
_original_record_loop = None
    ...
    def wrapped_get_observation() -> dict[str, Any]:
        ...
        if _ros_node is not None:
            try:
                ...
                # Don't let ROS2 publishing errors crash the record loop
...
def patched_record_loop(
    robot, events, fps,
    teleop_action_processor, robot_action_processor, robot_observation_processor,
    dataset=None, teleop=None, policy=None,
    preprocessor=None, postprocessor=None,
    control_time_s=None, single_task=None,
    display_data=False, display_compressed_images=False,
):
    """
    Patched version of lerobot's record_loop that wraps robot.get_observation
    to publish JointState and DiagnosticArray messages.

    All arguments are passed through to the original record_loop unchanged.
    """
    # Use the saved reference to avoid infinite recursion
    # (the module-level record_loop has been replaced with this function)
    original_record_loop = _original_record_loop
    ...
    try:
        # Call the original record_loop with the patched robot
...
def main():
    """Main entry point that initializes ROS2, patches lerobot, and runs inference/record."""
    global _ros_node, _original_record_loop
    ...
    logger.info('ROS2 inference/record wrapper starting')
    ...
    try:
        ...
        # Save original and monkey-patch the record_loop function
        _original_record_loop = lerobot_record.record_loop
        lerobot_record.record_loop = patched_record_loop

        logger.info('Monkey-patched lerobot_record.record_loop')
        logger.info('Starting lerobot inference/record (will publish to /joint_states and /servo_diagnostics)')
        ...
        logger.error(f'Error during inference/record: {e}', exc_info=True)
        raise
    finally:
        # Clean up ROS2
        ...
        rclpy.shutdown()
        logger.info('ROS2 inference/record wrapper stopped')
